(function (nope, wnd, gbl, none) {
    ...
    var report = new function () {
        ...
        function getData(model) {
            ...
            dataSource.each(function () {
                ...
                columns.each(function () {
                    ...
                    if (this.onxlsx) {
                        item[this.field]= this.onxlsx.call(data, this, text);
                    } else {
                        item[this.field] =  text;
                    }
                });
                list.push(item);
            });
        ...
        function exportToCSV(model) {
            var dataSource = getData(model);
            if (dataSource.length > 0) {
                console.log(['dataSource', dataSource]);
                var name = (model.name || 'Report');
                var myFile = name + ".xlsx";
                var myWorkSheet = XLSX.utils.json_to_sheet(dataSource);
                var myWorkBook = XLSX.utils.book_new();
                XLSX.utils.book_append_sheet(myWorkBook, myWorkSheet, name);
                XLSX.writeFile(myWorkBook, myFile);
            } else {
                alert('No Data Found.');
            }
            //console.log(['file, myWorkBook, myWorkSheet', file, myWorkBook, myWorkSheet]);
        };
        function exportToXl() {
        ...
        this.Bind = function (model) {
            ...
            if (model.Printable) {
                var container = model.Printable.container, btn = model.Printable.xlsx || '<a class="btn btn-white btn-default btn_export_xlsx btn-round pull-right margin-l-5 margin-r-5"><span class="glyphicon glyphicon-export"></span> Export To xlsx</a>';
                if (!container) {
                    var elm = $('<div class="empty_style button_container row"></div>');
                    model.elm.prepend(elm);
                    btn = $(btn);
                    elm.prepend(btn);
                    btn.click(model.Print);
                    return;
                ...
                Global.Click(elm, exportToCSV, model);
            }
        };
    };
    var printService = new function () {
        ...
        function gridPageFooter(model, columns) {
            var ttl = '', printable = model.Printable;
            printable.gridpagefooter = printable.gridpagefooter || IqraConfig.Grid.Printable.gridPageFooter || nope;
            if (typeof (printable.gridpagefooter) == funcType) {
                ttl = printable.gridpagefooter(model, columns);
            } else if (printable.gridpagefooter) {
                ttl = printable.gridpagefooter;
            }
            console.log(['model, columns', model, columns]);
            return ttl;
        };
        ...
        function setPeriodic(model, mywindow) {
            ...
            var header = '', fltr = [];
            if (typeof (model.Printable.periodic) === typeof (nope)) {
                header += model.Printable.periodic(model);
            }else if (model.Printable.periodic) {
            ...
            if (model.page && model.page.filter && model.page.filter.length) {
                var periodic = model.Printable.periodic;
                if (model.Printable.periodic && periodic.model && periodic.model.field) {
        ...
        function PrintElem(model) {
            ...
            mywindow.document.write('<table style="border-collapse: collapse; width: 100%;">');
            printHeader(model, mywindow, columns);
            printData(model, mywindow, columns);
            mywindow.document.write('</tbody><tfoot><tr><td colspan="200">' + gridPageFooter(model, columns) +'</td></tr></tfoot></table>');
    ...
    var gridBody = new function () {
        ...
        this.Reload = function (model, func) {
            if (model.dataSource && typeof model.dataSource == typeof []) {
                ...
                model.loading = true;
                return Global.CallServer(getUrl(model), function (response, status, xhr) {
                    model.loading = false;
                    if (!response.IsError) {
                        model.DataLoadedAt = new Date(xhr.getResponseHeader("date"));
                        model.ondata && model.ondata(response, model, status, xhr);
                        response = getColumnData(response);
                        model.dataBinding(response, status, xhr);
                        //console.log(['response, status, xhr', response, status, xhr]);
        ...
        function createView(elm, model, fieldModel, view) {
            ...
            dataModel.Model = {
                ...
                change: function (data) {
                    ...
                    if (data) {
                        fieldModel.Operation = fieldModel.operation = parseFloat(data.value, 10);
                    } else {
                        if (typeof fieldModel.operation != typeof none) {
    ...
    (function () {
        ...
        function setSummary(model) {
            ...
            model.summary.items = model.summary.items||[]
